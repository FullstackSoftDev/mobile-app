...
export async function getVacationByUserID(req, res) {
    try {
        const { user_id } = req.params;
        const result = await sql`SELECT * FROM vacation WHERE user_id = ${user_id} ORDER BY created_at DESC`;
...
export async function createVacation(req, res) {
    try {
        ...
        const vacation = await sql`
                ...
                VALUES (${user_id}, ${first_name}, ${last_name}, ${type}, ${note}, ${start_date}, ${end_date})
                RETURNING *
            `;
...
export async function deleteVacationByUserID(req, res) {
    try {
        const { id } = req.params;
        ...
        const result = await sql`DELETE FROM vacation WHERE id = ${id} RETURNING *`;
